...
VARIANTS=(rotvec quat rot6d z6d)
...
for minloss, k, best_v in sorted(hard, reverse=True)[:20]:
    r = meta[k]
    vals = [f(by_prim[k][v]["loss_final_best"]) for v in ["rotvec","quat","rot6d","z6d"]]
