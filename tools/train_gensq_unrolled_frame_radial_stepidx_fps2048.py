...
def unroll(args, X_world, R_gt, frame_net, eps_net, scale_net):
    ...
    for k in range(args.unroll_steps):
        ...
        # frame-first ordering
        X_frame_in = X_world @ frame_R.detach()
        target_delta = frame_R.detach().transpose(1, 2) @ R_gt
        delta_R, neff, maxw = frame_net(X_frame_in)
        ...
        # scale/eps see corrected canonical cloud in same step
...
def main():
    ...
    lines = [
        "experiment=unrolled_frame_radial_stepidx_fps2048_to_512",
