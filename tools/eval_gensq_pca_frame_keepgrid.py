...
def resample_observed(world, mode, keep_frac=0.5):
    ...
    for b in range(B):
        ...
        if mode == "dropout":
            ...
            # Keep the most visible cap according to a random view direction.
            # This patched version makes keep_frac active for viewcrop too.
            k = max(8, int(round(keep_frac * N)))
        ...
        choice = idx[torch.randint(0, idx.numel(), (N,), device=device)]
        out.append(pts[choice])
