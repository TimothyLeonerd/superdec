...
def fps_downsample_with_idx(points, m):
    B, N, _ = points.shape
    ...
    return points[batch[:, None], idx], idx
...
def make_batch(args, B, device):
    ...
    dirs_dense = dirs_src[None, :, :].expand(B, args.source_points, 3)

    X_dense = base.sample_gensq_batched_dirs(scale_gt, eps_gt, dirs_dense)
    X_obs, idx = fps_downsample_with_idx(X_dense, args.points)

    batch = torch.arange(B, device=device)
    dirs_obs = dirs_dense[batch[:, None], idx]

    # Sanity: X_obs == sample(gt params, selected rays)
    return X_obs, dirs_obs, scale_gt, eps_gt
...
def unroll(args, X, dirs_obs, scale_gt, eps_gt, eps_net, scale_net):
    B = X.shape[0]
    device = X.device
...
def train_loss(args, X, dirs_obs, scale_gt, eps_gt, hist):
    log_scale, raw_eps = hist[-1]

    if args.mode == "eps":
        Y = sample_pred(args, scale_gt.log(), raw_eps, dirs_obs)
        return chamfer_sq(Y, X).mean()

    if args.mode == "scale":
        raw_gt = base.exp_to_raw(eps_gt, args.exp_min, args.exp_max)
        Y = sample_pred(args, log_scale, raw_gt, dirs_obs)
        return chamfer_sq(Y, X).mean()
...
def mean_cat(xs):
    return float(torch.cat(xs).float().mean())


@torch.no_grad()
def evaluate(args, eps_net, scale_net, val_data, device):
    eps_net.eval()
    scale_net.eval()

    X_all, D_all, scale_all, eps_all = val_data
    loader = DataLoader(TensorDataset(X_all, D_all, scale_all, eps_all), batch_size=args.batch_size)

    rows = [{"cd": [], "scale_l1": [], "eps_l1": [], "log_eps_l1": []} for _ in range(args.unroll_steps + 1)]

    for X, D, scale_gt, eps_gt in loader:
        X = X.to(device)
        D = D.to(device)
        scale_gt = scale_gt.to(device)
        eps_gt = eps_gt.to(device)

        hist = unroll(args, X, D, scale_gt, eps_gt, eps_net, scale_net)

        for j, (log_scale, raw_eps) in enumerate(hist):
            Y = sample_pred(args, log_scale, raw_eps, D)
            cd = chamfer_sq(Y, X)
    ...
    for j in range(args.unroll_steps + 1):
        out.append({k: mean_cat(v) for k, v in rows[j].items()})
    return out
...
def main():
    ...
    with open(out / "metrics.csv", "w", newline="") as f:
        ...
        for ep in range(1, args.epochs + 1):
            ...
            for _ in range(steps_per_epoch):
                ...
                X, D, scale_gt, eps_gt = make_batch(args, b, device)
                hist = unroll(args, X, D, scale_gt, eps_gt, eps_net, scale_net)
                loss = train_loss(args, X, D, scale_gt, eps_gt, hist)
            ...
            ev = evaluate(args, eps_net, scale_net, val_data, device)
            final = ev[-1]
    ...
    lines = [
        f"experiment=fps2048_to_512_{args.mode}_oracle_selectedrays",
        f"mode={args.mode}",
        "prediction_rays=FPS_selected_dense_rays",
        f"source_points={args.source_points}",
        f"observed_points={args.points}",
        f"unroll_steps={args.unroll_steps}",
        f"best_cd_epoch={best_cd[1]} best_cd={best_cd[0]:.10f}",
        f"best_scale_epoch={best_scale[1]} best_scale_l1={best_scale[0]:.10f}",
        f"best_eps_epoch={best_eps[1]} best_eps_l1={best_eps[0]:.10f}",
        "",
    ]
    ...
    for name, best in [("best_cd", best_cd), ("best_scale", best_scale), ("best_eps", best_eps)]:
        ...
        for j, d in enumerate(ev):
            lines.append(
                f"step {j}: cd={d['cd']:.10f} "
                f"scale_l1={d['scale_l1']:.10f} "
                f"eps_l1={d['eps_l1']:.10f} "
                f"log_eps_l1={d['log_eps_l1']:.10f}"
            )
        lines.append("")
