...
from third_party.vnn.models.vn_layers import (
    VNLinear,
    VNLinearLeakyReLU,
    VNBatchNorm,
    VNStdFeature,
...
class VNStdFrameNet(nn.Module):
    def __init__(self, n_knn=20):
        super().__init__()
        self.n_knn = n_knn
        ...
        # official-ish VN-PointNet stem
        self.conv_pos = VNLinearLeakyReLU(3, c64, dim=5, negative_slope=0.0)
        self.conv1 = VNLinearLeakyReLU(c64, c64, dim=4, negative_slope=0.0)
        self.conv2 = VNLinearLeakyReLU(c64 * 2, c128, dim=4, negative_slope=0.0)
        self.conv3 = VNLinear(c128, c1024)
        self.bn3 = VNBatchNorm(c1024, dim=4)

        # difference from official classifier:
        # make one global frame from global VN features
    ...
    def forward(self, points):
        ...
        # official uses graph feature with cross-product channels
        feat = get_graph_feature_cross(x, k=self.n_knn)  # [B,3,3,N,k]
        x = self.conv_pos(feat)

        # pool over k-neighbors
        x = x.mean(dim=-1)  # [B,c64,3,N]

        x1 = self.conv1(x)
        x = torch.cat([x1, x1.mean(dim=-1, keepdim=True).expand_as(x1)], dim=1)
        x = self.conv2(x)

        x = self.bn3(self.conv3(x))  # [B,c1024,3,N]

        # official trick: concatenate global mean feature before std frame
        x_mean = x.mean(dim=-1, keepdim=True).expand_as(x)
        x = torch.cat([x, x_mean], dim=1)  # [B,2*c1024,3,N]

        # global VN feature
        x_global = x.mean(dim=-1)  # [B,2*c1024,3]

        _x_std, trans = self.std_feature(x_global)  # trans: [B,3,3], columns are frame axes
        R = project_to_so3(trans)
        return R
...
def main():
    ...
    if device.type != "cuda":
        raise RuntimeError("VN util currently expects CUDA because vendored get_graph_feature_cross hardcodes cuda.")
    ...
    model = VNStdFrameNet(n_knn=args.n_knn).to(device)
