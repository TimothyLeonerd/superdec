#!/usr/bin/env python3
"""Fixed-rotation SQ-Zero eps/scale/translation identifiability diagnostic.
...
Main diagnostic question:
    If GT rotation is fixed but eps, scale, and translation are free, can Chamfer
    still recover eps, or can translation/scale compensate for wrong eps while
    keeping low loss?

It reads the existing SQ-Zero LMDB sidecars, extracts individual GT primitives,
and optimizes eps, optionally scale and translation, from one or more
...
def optimize_one(
    ...
    assert final_eps is not None and final_scale is not None and final_trans is not None and loss_initial is not None
...
def main() -> None:
    ...
    print("=== SQ-Zero fixed-rotation eps/scale/translation identifiability diagnostic ===", flush=True)
    ...
    print(
        ...
        f"trans_initializations={len(trans_offsets)}",
        flush=True,
    )
        ...
        for init_pair in init_pairs:
            for init_scale_mult in scale_mults:
                for init_trans_offset in trans_offsets:
                    rows.append(optimize_one(rec, init_pair, init_scale_mult, init_trans_offset, args, eta=eta, omega=omega))
