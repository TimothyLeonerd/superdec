...
class EpsUnrollNet(nn.Module):
    """
    Eps delta predictor with unroll-index conditioning.
    Input per point:
      X / current_scale, current raw_eps, current log_scale, step_t
    Output:
      delta_raw_eps
    """
    def __init__(self):
...
class ScaleUnrollNet(nn.Module):
    """
    Scale delta predictor with unroll-index conditioning.
    Input per point:
      raw X, current log_scale, current raw_eps, step_t
    Output:
      delta_log_scale
    """
    def __init__(self):
...
def unroll(args, X, eps_net, scale_net):
    ...
    for k in range(args.unroll_steps):
        ...
        delta_raw_eps = eps_net(X_scaled, raw_eps, log_scale.detach(), step_t)
        ...
        delta_log_scale = scale_net(X, log_scale, raw_eps.detach(), step_t)
...
def final_losses(args, X, log_scale_N, raw_eps_N, dirs_base):
    ...
    # Scale loss: updates ScaleNet path only.
    loss_scale = full_cd(args, X, scale_N, raw_eps_N.detach(), dirs_base).mean()

    # Eps loss: updates EpsNet path only.
    loss_eps = full_cd(args, X, scale_N.detach(), raw_eps_N, dirs_base).mean()

    # Optional mild bound/smoothness regularization.
...
def main():
    ...
    scale_net = ScaleUnrollNet().to(device)
    ...
    lines = []
    lines.append("experiment=unrolled_coupled_final_cd_stepidx")
