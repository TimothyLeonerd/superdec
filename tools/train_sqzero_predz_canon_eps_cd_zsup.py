...
def main():
    ...
    print(f"[train] {tuple(Xtr.shape)} [val] {tuple(Xva.shape)} w_z={args.w_z}")
    ...
    with open(out / "metrics.csv", "w", newline="") as f:
        ...
        for ep in range(1, args.epochs + 1):
            ...
            for x, eps_gt, z_gt in train_loader:
                x, eps_gt, z_gt = x.to(device), eps_gt.to(device), z_gt.to(device)
                eps_pred, z_pred = model(x)
                ...
                cd_loss = chamfer_sq(pred_surf, gt_surf).mean()
                z_loss = z_axis_loss(z_pred, z_gt).mean()
                loss = cd_loss + args.w_z * z_loss

                opt.zero_grad(set_to_none=True)
                loss.backward()
                opt.step()

                total += cd_loss.item() * x.shape[0]
                seen += x.shape[0]
