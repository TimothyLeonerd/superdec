#!/usr/bin/env python3
"""Fixed-rotation/translation SQ-Zero eps/scale identifiability diagnostic.

This script does NOT train SuperDec. It directly optimizes per-primitive SQ
parameters with PyTorch autograd.

Default question:
    If GT scale / rotation / translation are fixed, can Chamfer recover eps?

Next diagnostic question with --optimize-scale:
    If GT rotation / translation are fixed but scale is free, can Chamfer still
    recover eps, or can scale compensate for wrong eps while keeping low loss?

It reads the existing SQ-Zero LMDB sidecars, extracts individual GT primitives,
and optimizes eps, optionally scale, from one or more initializations. It also
writes optional eps-loss landscape plots.
"""
...
def main() -> None:
    ...
    print(f"eps_initializations={len(init_pairs)} scale_initializations={len(scale_mults)}", flush=True)
    ...
    for rec_i, rec in enumerate(records):
        ...
        for init_pair in init_pairs:
            for init_scale_mult in scale_mults:
                rows.append(optimize_one(rec, init_pair, init_scale_mult, args, eta=eta, omega=omega))
