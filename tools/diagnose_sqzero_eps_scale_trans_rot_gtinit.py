...
def main() -> None:
    ...
    parser.add_argument(
        "--include-gt-eps-init",
        action="store_true",
        help="Also initialize eps exactly at each primitive's GT eps.",
    )
    ...
    for rec_i, rec in enumerate(records):
        ...
        rec_init_pairs = list(init_pairs)
        if args.include_gt_eps_init:
            rec_init_pairs.append((
                float(rec.shape[0].detach().cpu().item()),
                float(rec.shape[1].detach().cpu().item()),
            ))

        for init_pair in rec_init_pairs:
