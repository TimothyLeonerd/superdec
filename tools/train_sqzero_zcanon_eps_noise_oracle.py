...
def perturb_z_fixed_angle(z, angle_deg, rng):
    """Perturb unit z by exactly angle_deg in a random tangent direction."""
    if angle_deg <= 0:
        return F.normalize(z, dim=0)

    z = F.normalize(z, dim=0)
    v = torch.from_numpy(rng.normal(size=3)).to(device=z.device, dtype=z.dtype)
    v = v - torch.dot(v, z) * z

    if torch.norm(v) < 1e-8:
        helper = torch.tensor([1.0, 0.0, 0.0], device=z.device, dtype=z.dtype)
        v = helper - torch.dot(helper, z) * z

    v = F.normalize(v, dim=0)
    theta = torch.tensor(float(angle_deg) * np.pi / 180.0, device=z.device, dtype=z.dtype)
    return F.normalize(torch.cos(theta) * z + torch.sin(theta) * v, dim=0)
...
def build_primitive_tensor(args, split, max_objects):
    ...
    for i in range(len(ds)):
        ...
        for k in range(K):
            ...
            # Use only GT z-axis, with controlled angular noise. No full GT rotation.
            z_gt = gt_rotate[k, :, 2]
            z_used = perturb_z_fixed_angle(z_gt, args.z_noise_deg, rng)
            Q = frame_from_z(z_used)
...
def main():
    ...
    ap.add_argument("--z-noise-deg", type=float, default=0.0)
