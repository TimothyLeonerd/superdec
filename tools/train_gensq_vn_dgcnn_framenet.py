...
from third_party.vnn.models.utils.vn_dgcnn_util import (
...
class VNDGCNNFrameNet(nn.Module):
    def __init__(self, n_knn=20, hidden=256):
        ...
        self.conv1 = VNLinearLeakyReLU(3, c1, dim=5, negative_slope=0.0)
        self.conv2 = VNLinearLeakyReLU(c1 * 2, c2, dim=5, negative_slope=0.0)
        self.conv3 = VNLinearLeakyReLU(c2 * 2, c3, dim=5, negative_slope=0.0)
        self.conv4 = VNLinearLeakyReLU(c3 * 2, c4, dim=5, negative_slope=0.0)
    ...
    def forward(self, points):
        ...
        g1 = get_graph_feature_cross(x0, k=self.n_knn)  # [B,3,3,N,k]
        x1 = self.edge_reduce(self.conv1(g1))           # [B,c1,3,N]

        g2 = get_graph_feature(x1, k=self.n_knn)
        x2 = self.edge_reduce(self.conv2(g2))

        g3 = get_graph_feature(x2, k=self.n_knn)
        x3 = self.edge_reduce(self.conv3(g3))

        g4 = get_graph_feature(x3, k=self.n_knn)
        x4 = self.edge_reduce(self.conv4(g4))

        feat = torch.cat([
            second_order_pool(x1).flatten(1),
            second_order_pool(x2).flatten(1),
            second_order_pool(x3).flatten(1),
            second_order_pool(x4).flatten(1),
        ], dim=1)
...
def main():
    ...
    if device.type != "cuda":
        raise RuntimeError("vendored get_graph_feature expects CUDA")
    ...
    model = VNDGCNNFrameNet(n_knn=args.n_knn, hidden=args.hidden).to(device)
