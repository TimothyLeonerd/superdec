...
def rotvec_to_quat(rotvec: torch.Tensor) -> torch.Tensor:
    ...
    if float(theta.detach().cpu().item()) < 1e-8:
        xyz = 0.5 * rotvec
        w = rotvec.new_tensor(1.0)
        return safe_normalize(torch.cat([w.view(1), xyz]))
    axis = rotvec / theta
    return torch.cat([torch.cos(half).view(1), torch.sin(half) * axis])


def quat_to_matrix(raw_quat: torch.Tensor) -> torch.Tensor:
    """Unconstrained quaternion parameter -> rotation matrix."""
    q = safe_normalize(raw_quat)
...
def rot6d_to_matrix(raw6: torch.Tensor) -> torch.Tensor:
    """Standard 6D rotation: raw x-ish/y-ish columns -> SO(3)."""
    a = raw6[0:3]
    b = raw6[3:6]
    x = safe_normalize(a)
    b_orth = b - torch.dot(b, x) * x
    y = safe_normalize(b_orth)
    z = torch.cross(x, y, dim=0)
    return torch.stack([x, y, z], dim=1)


def z6d_to_matrix(raw6: torch.Tensor) -> torch.Tensor:
    """z-prioritized 6D rotation: raw z-axis + helper x-ish vector -> SO(3)."""
    z = safe_normalize(raw6[0:3])
    h = raw6[3:6]
    h_orth = h - torch.dot(h, z) * z
    x = safe_normalize(h_orth)
...
def matrix_to_z6d(R: torch.Tensor) -> torch.Tensor:
    return torch.cat([R[:, 2], R[:, 0]], dim=0)


def init_rotation_param_from_rotvec(rotvec: torch.Tensor, rotation_param: str) -> torch.Tensor:
    """Initialize every representation to the same delta rotation."""
    R = rodrigues(rotvec)
    if rotation_param == "rotvec":
        return rotvec.detach().clone()
    if rotation_param == "quat":
        return rotvec_to_quat(rotvec).detach().clone()
    if rotation_param == "rot6d":
        return matrix_to_rot6d(R).detach().clone()
    if rotation_param == "z6d":
        return matrix_to_z6d(R).detach().clone()
    if rotation_param == "lie":
        return torch.zeros(3, device=rotvec.device, dtype=rotvec.dtype)
    raise ValueError(f"Unknown rotation_param: {rotation_param!r}")


def rotation_param_to_matrix(raw: torch.Tensor, rotation_param: str) -> torch.Tensor:
    if rotation_param == "rotvec":
        return rodrigues(raw)
    if rotation_param == "quat":
        return quat_to_matrix(raw)
    if rotation_param == "rot6d":
        return rot6d_to_matrix(raw)
    if rotation_param == "z6d":
        return z6d_to_matrix(raw)
    if rotation_param == "lie":
        return rodrigues(raw)
    raise ValueError(f"Unknown rotation_param: {rotation_param!r}")
...
def optimize_one(
    ...
    init_delta_R = rodrigues(init_rotvec).detach()
    raw_rot_param = None
    lie_base_R = init_delta_R.clone()
    if args.optimize_rotate:
        raw_rot_param = init_rotation_param_from_rotvec(init_rotvec, args.rotation_param).requires_grad_(True)
        params.append(raw_rot_param)
    ...
    def current_rotate() -> torch.Tensor:
        if raw_rot_param is None:
            return rec.rotate
        if args.rotation_param == "lie":
            return rodrigues(raw_rot_param) @ lie_base_R @ rec.rotate
        return rotation_param_to_matrix(raw_rot_param, args.rotation_param) @ rec.rotate
    ...
    for step in range(args.steps + 1):
        ...
        if (
            raw_rot_param is not None
            and args.rotation_param == "lie"
            and args.lie_commit_every > 0
            and (step + 1) % args.lie_commit_every == 0
        ):
            with torch.no_grad():
                lie_base_R = project_to_so3(rodrigues(raw_rot_param.detach()) @ lie_base_R)
                raw_rot_param.zero_()
            opt.state.pop(raw_rot_param, None)
    ...
    init_rotate = init_delta_R @ rec.rotate
...
def main() -> None:
    ...
    parser.add_argument(
        "--rotation-param",
        default="rotvec",
        choices=["rotvec", "quat", "rot6d", "z6d", "lie"],
        help=(
            "Rotation delta parameterization used when --optimize-rotate is set. "
            "All variants are initialized from --init-rotvecs-deg to the same delta rotation."
        ),
    )
    parser.add_argument(
        "--init-rotvecs-deg",
    ...
    parser.add_argument("--lie-commit-every", type=int, default=5)
    ...
    print(
        f"optimize_rotate={args.optimize_rotate} rotation_param={args.rotation_param} "
        f"init_rotvecs_deg={args.init_rotvecs_deg}",
        flush=True,
    )
