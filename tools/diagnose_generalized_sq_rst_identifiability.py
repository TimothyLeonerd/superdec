...
def sample_generalized_surface(scale, exp, dirs, newton_iters=12):
    # scale [B,3], exp [B,3], dirs [S,3], returns [B,S,3]
    B = scale.shape[0]
    S = dirs.shape[0]
...
def main():
    ...
    opt = torch.optim.Adam([raw_rot, raw_exp], lr=args.lr)
    ...
    for step in range(1, args.steps + 1):
        ...
        pred_local = sample_generalized_surface(gt_scale, pred_exp, dirs)
