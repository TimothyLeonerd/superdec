...
class ScaleUnrollNet(nn.Module):
    """
    Scale delta predictor with minimal exponent-undo input.

    Per-point input:
      v = sign(u) * |u|^(eps/2), where u = X / scale
      current log_scale
      current raw_eps

    Input dim:
      v(3) + log_scale(3) + raw_eps(3) = 9
    """
    ...
    def forward(self, X, log_scale, raw_eps):
        ...
        v = torch.sign(u) * u.abs().clamp_min(1e-8).pow(eps[:, None, :] / 2.0)

        ls = log_scale[:, None, :].expand(B, S, 3)
        re = raw_eps[:, None, :].expand(B, S, 3)

        feat = torch.cat([v, ls, re], dim=-1)
        h = self.point(feat).amax(dim=1)
        return self.head(h)
...
def main():
    ...
    lines = []
    lines.append("experiment=unrolled_coupled_final_epsray_vonly")
