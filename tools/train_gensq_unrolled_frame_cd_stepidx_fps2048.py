...
def fps_downsample(points, m):
    ...
    # random start; fixed by global torch seed
...
def make_batch(args, B, dirs_base, device):
    """
    Generate dense source surface, then FPS downsample to observed 512 points.
    dirs_base is still used elsewhere for prediction sampling.
    """
    S_src = args.source_points
    ...
    dirs_src = base.fibonacci_sphere(S_src, device)
    dirs = dirs_src[None, :, :].expand(B, S_src, 3)

    X_local_dense = base.sample_gensq_batched_dirs(scale_gt, eps_gt, dirs)

    R_gt = random_rotations(B, device)
    X_world_dense = X_local_dense @ R_gt.transpose(1, 2)

    if args.points < S_src:
        X_world = fps_downsample(X_world_dense, args.points)
    else:
        X_world = X_world_dense
...
def main():
    ...
    lines = [
        "experiment=unrolled_frame_cd_stepidx_fps2048_to_512",
