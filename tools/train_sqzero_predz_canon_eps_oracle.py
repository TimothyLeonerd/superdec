...
def build_primitive_tensor(args, split, max_objects):
    ...
    for i in range(len(ds)):
        ...
        for k in range(K):
            ...
            # GT translation removed. Only scalar GT size normalization.
            size = torch.clamp(gt_scale[k].mean(), min=1e-6)
            x = (p - gt_trans[k]) / size

            z = F.normalize(gt_rotate[k, :, 2], dim=0)
...
def frame_from_z_batch(z):
    ...
    return torch.stack([x, y, z], dim=-1)  # [B,3,3], columns are axes
...
class PredZCanonEpsNet(nn.Module):
    ...
    def forward(self, x):
        z_raw = self.z_head(self.z_enc(x))
        z = F.normalize(z_raw, dim=-1)

        Q = frame_from_z_batch(z)
        x_canon = torch.einsum("bnd,bdk->bnk", x, Q)

        eps_raw = self.eps_head(self.eps_enc(x_canon))
        eps = 0.1 + 1.8 * torch.sigmoid(eps_raw)
        return eps, z


def z_loss(z_pred, z_gt):
...
def evaluate(model, loader, device):
    ...
    for x, eps, z in loader:
        ...
        z_losses.append(z_loss(pred_z, z).cpu())
...
def main():
    ...
    with open(out / "metrics.csv", "w", newline="") as f:
        ...
        for ep in range(1, args.epochs + 1):
            ...
            score = va["eps_l1_mean"] + 0.001 * va["z_angle_mean"]
            if score < best:
                best = score
            ...
            print(
                ...
                f"z_angle={va['z_angle_mean']:.2f} "
                f"z_p95={va['z_angle_p95']:.2f}",
                flush=True,
            )
